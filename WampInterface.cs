...
//    .WebSocketTransport(new Uri("ws://localhost:443/ws"))
//    .JsonSerialization()
//    .Build());

//IWampClientConnectionMonitor monitor = channel.RealmProxy.Monitor;

//monitor.ConnectionBroken += (sender, args) => { 
//   Program.ReportAndLog(" Connection Broken - event monitor triggered."); 
//};
//monitor.ConnectionError += (sender, args) => { 
//   Program.ReportAndLog(" Connection Error - event monitor triggered.");
//};

//await channel.Open().ConfigureAwait(false);
//Program.ReportAndLog("channel.Open.ConfigureAwait has returned.");
...
//////////WampChannel = channelFactory.CreateChannel(
//////////    WampServerRealm, WebSocketTransport<new Uri(WampServerURL)>, binding);

//////    .WebSocketTransport(new Uri(WampServerURL))
//////    .JsonSerialization()
//////    .Build();
//////Program.ReportAndLog($"{stepDescription} - URL: {WampServerURL}    REALM: {WampServerRealm}");